	...
	public static void main(String[]args){
		if (args.length < 2) {
			...
			if(args.length < 3 || args[2].equals("random")){
				for (int i = 0; i < randData.length; i++) randData[i] = rng.nextInt();
			}else if(args[2].equals("equal")){
				int eq = rng.nextInt();
				for (int i = 0; i < randData.length; i++) randData[i] = eq;
			}else if(args[2].equals("sorted")){
				int eq = rng.nextInt();
				for (int i = 0; i < randData.length; i++) randData[i] = i + eq;
			}else if(args[2].equals("reversed")){
				int eq = rng.nextInt();
				for (int i = 0; i < randData.length; i++) randData[i] = eq - i;
			}
			...
			//This is not meant to be a timed test...
				...
				if( equals(randData,randDataSelection)){
				 ...
				 System.out.println(Arrays.toString(randDataSelection));
				}

				if( equals(randData,randDataInsertion)){
